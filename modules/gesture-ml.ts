...
function computeAxisStats(values: number[]): AxisStats {
  if (!values.length) {
    return { mean: 0, std: 0, min: 0, max: 0, range: 0, energy: 0, avgAbsDiff: 0 };
  }
...
export function movingAverage(samples: SensorSample[], window = 3): SensorSample[] {
...
export function extractFeatureVector(samples: SensorSample[]): FeatureExtraction {
  const sampleCount = samples.length;
  const durationMs = sampleCount ? samples[sampleCount - 1].t - samples[0].t : 0;
  ...
  const statKeys: (keyof AxisStats)[] = ['mean', 'std', 'min', 'max', 'range', 'energy', 'avgAbsDiff'];
  ...
  const sampleRate = durationMs > 0 ? (sampleCount / durationMs) * 1000 : sampleCount;
...
export function trainSoftmaxModel(
  ...
  const normalized = entries.map((entry) =>
    entry.values.map((value, idx) => (value - featureMeans[idx]) / featureStd[idx])
  );

  const weights = Array.from({ length: labelCount }, () => new Array(featureCount).fill(0));
  const biases = new Array(labelCount).fill(0);
  const lossHistory: number[] = [];

  for (let epoch = 0; epoch < epochs; epoch++) {
    const gradW = Array.from({ length: labelCount }, () => new Array(featureCount).fill(0));
    const gradB = new Array(labelCount).fill(0);
    let loss = 0;

    normalized.forEach((row, rowIdx) => {
      const logits = weights.map((w, clsIdx) =>
        w.reduce((sum, weight, featIdx) => sum + weight * row[featIdx], biases[clsIdx])
      );
...
export function predictFromModel(model: GestureModel, values: number[]): PredictionResult {
  const { featureMeans, featureStd, weights, biases, labels } = model;
  if (!labels.length) {
    throw new Error('Model is empty');
  }
  const normalized = values.map((value, idx) => (value - featureMeans[idx]) / featureStd[idx]);
  const logits = weights.map((w, clsIdx) =>
    w.reduce((sum, weight, featIdx) => sum + weight * normalized[featIdx], biases[clsIdx])
  );
  const probs = softmax(logits);
  const bestIdx = probs.indexOf(Math.max(...probs));
  const distribution = labels.map((label, idx) => ({ label, confidence: probs[idx] }));
